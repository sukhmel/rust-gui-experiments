...
fn patch_gpui_shaders() {
    ...
    let shader_pattern = format!(
        "{}/registry/src/index.crates.io-*/gpui-*/src/platform/mac/shaders.metal",
        cargo_home
    );
    ...
    match glob::glob(&shader_pattern) {
        Ok(entries) => {
            for entry in entries.flatten() {
                if let Err(e) = patch_shader_file(&entry) {
                    println!("cargo:warning=Failed to patch {:?}: {}", entry, e);
                }
            }
        }
        Err(e) => {
            println!("cargo:warning=Failed to glob pattern {}: {}", shader_pattern, e);
        }
    }
}
...
#[cfg(all(target_os = "macos", feature = "gpui"))]
fn patch_shader_file(path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
